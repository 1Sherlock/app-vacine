/**
 * Created by Sherlock on 06.09.2021.
 */
...
const ViewResult = (props) => {
    useEffect(()=>{
        props.getResult(props.match.params.id);
    }, [])
        ...
        <div id="forDashboard">
            <div className="border-bottom">
                <nav className="navbar navbar-expand-md container navbar-light my-2">
                    <p className="navbar-brand font-weight-bold text-primary mb-0">Covid-19</p>
                </nav>
            </div>
                ...
                <div className="container bg-white border p-md-4 p-3">
                    ...
                    <div className="">
                        <table className="table table-striped table-bordered">
                            ...
                            {/*    <td>{props.result?.address ? props.result?.address : "-"}</td>*/}
                            {/*</tr>*/}
                            </tbody>
                        </table>
                    </div>

                    <h4 className="mb-3">Вакцина</h4>
                    ...
                    <div className="">
                        <table className="table table-striped table-bordered">
                            ...
                            <tbody>
                            <tr>
                                <td>{props.result?.serial}</td>
                                <td>{props.result?.analiseDate}</td>
                                <td>{props.result?.status}</td>
                                <td>{props.result?.methodRu} / {props.result?.methodEn}</td>
                                <td>{props.result?.laboratory?.nameRu} / {props.result?.laboratory?.nameEn}
                                </td>
                                <td>
                                    {props.result?.place?.nameRu} / {props.result?.place?.nameEn}
                                </td>
                                <td className="text-center text-primary">
                                    <a href={"https://webtopdf.expeditedaddons.com/?api_key=53YH4OUM7G8Q9BJI4NVF6CW590X0SDZT7L1KEA8R3621P2&content=https://app-vacinee.herokuapp.com/sertificate/" + props.result?.id  + "&margin=3&html_width=1040&title=Sertificate"} target="_blank">                                        <svg viewBox="64 64 896 896" focusable="false" className="" data-icon="file-pdf"
...
export default connect(mapStateToProps, {getResult})(ViewResult);